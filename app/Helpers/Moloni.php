...
class Moloni
{
    ...
    public function login(){
        $response = Http::get($this->_endpointUrl."grant/?grant_type=password&client_id=".config('moloni.client_id')."&username=".config('moloni.email')."&client_secret=".config('moloni.client_secret')."&password=".urlencode(config('moloni.password')));
    ...
    public function refreshToken($token = null, $refresh_token = null){
        ...
        $response = Http::post($this->_endpointUrl."grant/?grant_type=refresh_token&client_id=".config('moloni.client_id')."&client_secret=".config('moloni.client_secret')."&refresh_token=$refresh_token");
    ...
    public function getOrInsertCustomer($name, $number, $address, $city , $vat='999999990' ,$zip=null, $email = null){
        $customerId=null;
        /*\Debugbar::error('customer by number : ',$this->getCustomerByNumber($number));
        if(($customers = $this->getCustomerByNumber($number)) == false){
            \Debugbar::error("Não achou cliente");
            \Debugbar::error("proximo if", empty($vat),$vat=="999999990",($customers = $this->getCustomerByVat($vat)) == false);*/
            ...
            if(empty($vat) || $vat=="999999990" || ($customers = $this->getCustomerByVat($vat)) == false){
                ...
                if(($data = $this->insertCustomer($name, $number, $address, $city, $vat, $zip, $email)) == false){
                    \Debugbar::error('insert customer',$data);
                    return false;
                }else{
                    if(empty($data['valid'])){
    ...
    /**
     * Get one documento related to a invoice
     * @param integer $documentId
     * @return boolean or array
     */
    public function getOneInvoiceReceipt($documentId){
        if($this->getToken() == false){
            return false;
        }else{
            $response = Http::post($this->_endpointUrl."documents/getOne/?access_token=$this->accessToken&json=true", [
                'company_id' => Setting::getParam('moloni_company_id'),
    ...
    public function insertInvoiceReceipt($customerId, $products, $payment,$orderId=null, $status=0, $sendEmail=null){
        \Debugbar::error('products',$products,'--');
        if($this->getToken() == false){
            return false;
        }else{
            $array = [
    ...
    public function checkValidResponse($response){
        if ($response->successful()) {
            \Debugbar::info('ora a response:',$response);
            $data = $response->json();
            \Debugbar::info($data);
            if(!empty($data['error'])){
                ...
                \Debugbar::info('vai retornar a resposta');
                return $data;
            }
        }else{
            \Debugbar::error("Deu erro");
            \Debugbar::error($response);
            \Debugbar::error($data = $response->json());
