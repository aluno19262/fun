...
class User extends Authenticatable implements MustVerifyEmail, HasMedia, Auditable
{
    ...
    public function associate(){
        return $this->hasOne(\App\Models\Associate::class, 'user_id');
    }

    /**
     * check if user has permission to edit associate fields
     * @return bool true/false
     */
    public function checkPermissionToEdit($associate){
        if($this->hasAnyRole('Staff|SuperAdmin') || (!empty($associate) && !empty($associate->user_id) && $associate->user_id == $this->id)){
            return true;
        }else{
            return false;
        }
