...
class UserDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatable =datatables()
            ...
            ->filter(function ($query) {
                \Debugbar::error(request()->get('columns'));
                if (!empty(request()->get('columns')[4]['search']['value'])) {
                    $search = str_replace(["^","$"], "", request()->get('columns')[4]['search']['value']);
                    $query->whereHas('roles', function($q) use($search){
                        $q->where('roles.id', $search);
                    });
                }
            })
    ...
    protected function getColumns()
    {
        $columns = [
            ...
            Column::make('role')->orderable(true)->title(__('Role'))->attributes(['data-options' => json_encode(User::getRolesArray())]),
        ];
