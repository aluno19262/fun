...
<div id="kt_header" style="" class="header {{ theme()->printHtmlClasses('header', false) }} align-items-stretch " {{ theme()->printHtmlAttributes('header') }}>
    ...
    <div class="{{ theme()->printHtmlClasses('header-container', false) }} d-flex align-items-stretch justify-content-between">
        ...
        @if (theme()->getOption('layout', 'aside/display') === true  && (!empty(auth()->user())  && (!auth()->user()->can('accessAsUser') || auth()->user()->canAny(['accessAsCAC','manageApp']))))
        ...
        @if (theme()->getOption('layout', 'aside/display') === false || (auth()->user()->can('accessAsUser') && !auth()->user()->canAny(['accessAsCAC','manageApp'])))
            <!--begin::Logo-->
                <div class="d-flex align-items-center flex-grow-1 flex-lg-grow-0 me-lg-15">
                    <a href="{{ route('home') }}">
                        <img alt="Logo" src="{{ asset(theme()->getMediaUrlPath() . 'logos/apap-logo.png') }}" class="h-75px"/>
                    </a>
                </div>
                <!--end::Logo-->
        @else
            <!--begin::Mobile logo-->
                <div class="d-flex align-items-center flex-grow-1 flex-lg-grow-0">
                    <a href="{{ route('home') }}" class="d-lg-none">
                        <img alt="Logo" src="{{ asset(theme()->getMediaUrlPath() . 'logos/') }}" class="h-15px"/>
                    </a>
                </div>
                <!--end::Mobile logo-->
        @endif
